...
public class JobsController extends
	...
	public void save(@RequestParam("name") String name, @RequestParam("definition") String definition,
		...
		if(ComposedJobConfigurer.isComposedJobDefinition(definition)){
			moduleDefinitionService.compose(ComposedJobConfigurer.getComposedJobModuleName(name), ModuleType.job,definition, false);
		}
	...
	public void delete(@PathVariable("name") String name) throws Exception {
		...
		if( moduleDefinitionService.findDefinition(
				ComposedJobConfigurer.getComposedJobModuleName(name), ModuleType.job) != null) {
			moduleDefinitionService.delete(ComposedJobConfigurer.getComposedJobModuleName(name), ModuleType.job);
		}
	}
